function [V,n,p,A,H] = tdrelaxpstep4(Vc,nc,pc,Ac,Hc,V_p,n_p,p_p,A_p,H_p,dtV,dtn,dtp,dtH,dtVp2,dtnp2,dtpp2,dtHp2,dt)

display(['  Start TD simulation relaxation4 per step:']);
                        ...
                        switch ajvolM_lk(j)
                            case 1
                                ...
                                dJv = dJndV(alpha_n,beta,n(n1),n(n2))...
                                    +dJpdV(alpha_p,beta,p(n1),p(n2)) + 3*epsilon_sd/(dt*linkL(lk));
                            ...
                            case 2
                                ...
                                dJv = sigma/linkL(lk) + 3*epsilon_mt/(dt*linkL(lk));
                                dJH = -sign_n1(i)* (sigma+3*epsilon_mt/dt);
                            case 3
                                J   = epsilon_in*dtE1;
    ...
    %%%%%%%%% build Jacobian and rhs of K (current continuity of semi) %%%%%%%%%%%%%
     if ~isempty(eqnSemiNodes)
        for n1 = eqnSemiNodes.'
            ...
            semiV = sum(ajvolV_n1(ajvolM_n1 == 1));
            JKn_n(n1,n1) = JKn_n(n1,n1)-3*semiV/dt;
            JKp_p(n1,n1) = JKp_p(n1,n1)+3*semiV/dt;
    ...
    %%%%%%%%%%%%% Build Jacobian and rhs of G (Ampere's law) %%%%%%%%%%%%%
     for l1 = eqnLinks'
        ...
        if ~isBndNodes(n1)
            ...
            JG_v(l1,n1) = JG_v(l1,n1)+3*coefV_n1/dt;
        ...
        if ~isBndNodes(n2)
            ...
            JG_v(l1,n2) = JG_v(l1,n2)-3*coefV_n2/dt;
            ...
            switch ajvolM_l1(i)
                case 1
                    ...
                    dJ_v = dJndV(alpha_n,beta,n(n1),n(n2))...
                          +dJpdV(alpha_p,beta,p(n1),p(n2))+3*epsilon_sd/(dt*linkL(l1));
                    dJn_ni = dJdci(alpha_n,beta,'n');
                    dJn_nj = dJdcj(alpha_n,beta,'n');
                    dJp_pi = dJdci(alpha_p,beta,'p');
                    dJp_pj = dJdcj(alpha_p,beta,'p');
                ...
                case 2
                    dJ_v = sigma/linkL(l1)+3*epsilon_mt/(dt*linkL(l1));
                    dJ_H = -(sigma+3*epsilon_mt/dt);
...
%%        rhs_T(l1)  = ( H(l1) + H_p(l1) + 4*Hc(l1) )*dt/3  + A_p(l1) - A(l1);
        rhs_T(l1)  = 2*H(l1)*dt - (dtH(l1)+4*dtH1(l1)+dtH2(l1)) *dt^2/3 + A_p(l1) - A(l1);
        JT_A(l1,l1)= -1;
        JT_H(l1,l1)= dt;
     end
...
%    dX = invJacob*rhs;
    ...
    dtV  = 3 * (V - V_p)/dt - 4*dtV1 - dtV2;
    dtH  = 3 * (H - H_p)/dt - 4*dtH1 - dtH2;
    dtn  = 3 * (n - n_p)/dt - 4*dtn1 - dtn2;
    dtp  = 3 * (p - p_p)/dt - 4*dtp1 - dtp2;
...
display(['  End TD simulation relaxation4 per step.']);
