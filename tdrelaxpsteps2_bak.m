function [V,A,H,mJ_0,mJ_1,mJ_2,mJ_1p,mJ_2p,dtV,dtH] = tdrelaxpstep2(V_p,A_p,H_p,Js,mJ_0p,mJ_1p,mJ_2p,mJ_1pp,mJ_2pp,dtV,dtH,dt)
...
tic;
    ...
    n1 = links(l1,1);
    n2 = links(l1,2);
    %drude mJ_(n+1)=(2-gamma_p*dt)/(2+gamma_p*dt)*mJ_n+ omega_p^2*dt*(E_n+1 + E_n)/(2+gamma_p*dt)
     mJ_0(l1) =(2-gamma_p*dt)/(2+gamma_p*dt)*mJ_0p(l1)+dt*omega_p*omega_p*((V(n1)+V_p(n1)-V(n2)-V_p(n2))/linkL(l1)-H(l1)-H_p(l1))/(2+gamma_p*dt);
   ...
   %or to save the storage, Lorentz mJ_(n+1)=alpha*mJ_n+beta*mJ_(n-1) + lgamma* (E_n+1 -E_n)/dt 
    mJ_1(l1)=(2-lomega_1*lomega_1*dt*dt)/(1+lgamma_1*dt)*mJ_1p(l1)+(lgamma_1*dt-1)/(1+lgamma_1*dt)*mJ_1pp(l1)+...
         (lepsr_1*lomega_1*lomega_1*dt*dt)/(1+lgamma_1*dt)/dt*((V(n1)-V(n2)-V_p(n1)+V_p(n2))/linkL(l1)-H(l1)+H_p(l1));

    mJ_2(l1)=(2-lomega_2*lomega_2*dt*dt)/(1+lgamma_2*dt)*mJ_2p(l1)+(lgamma_2*dt-1)/(1+lgamma_2*dt)*mJ_2pp(l1)+...
         (lepsr_2*lomega_2*lomega_2*dt*dt)/(1+lgamma_2*dt)/dt*((V(n1)-V(n2)-V_p(n1)+V_p(n2))/linkL(l1)-H(l1)+H_p(l1));
    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   for k=1:NeqnNodes
        n1 = eqnNodes(k);
                        ...
                        switch ajvolM_lk(j)
                  		...
                  		J=sign_n1(i)*(mJ_0(lk)+mJ_1(lk)+mJ_2(lk))+epsilon_mt*dtE1;
                                dJv = sign_n1(i)*(dt*omega_p*omega_p/(dt*gamma_p+2)/linkL(lk)+...
				                 (lepsr_1*lomega_1*lomega_1*dt*dt)/(1+lgamma_1*dt)/dt/linkL(lk) +...
				                 (lepsr_2*lomega_2*lomega_2*dt*dt)/(1+lgamma_2*dt)/dt/linkL(lk)) +...
				      epsilon_mt/(0.5*dt*linkL(lk));
                                dJH = -sign_n1(i)*(dt*omega_p*omega_p/(dt*gamma_p+2) +...
				                  (lepsr_1*lomega_1*lomega_1*dt*dt)/(1+lgamma_1*dt)/dt +...
				                  (lepsr_2*lomega_2*lomega_2*dt*dt)/(1+lgamma_2*dt)/dt +...
						  epsilon_mt/(0.5*dt));
                            case 2
                                J   = epsilon_in*dtE1;
	...
	%if both nodes are on surface,no Lorentz
        ...
        for i = 1:length(ajvol_l1)
            switch ajvolM_l1(i)
			  ...
			  (lepsr_1*lomega_1*lomega_1*dt*dt)/(1+lgamma_1*dt)/dt/linkL(l1) +...
			  (lepsr_2*lomega_2*lomega_2*dt*dt)/(1+lgamma_2*dt)/dt/linkL(l1)) +...
