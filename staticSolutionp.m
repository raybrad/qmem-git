...
%    JF_p = []; JKn_p = []; JKp_v = []; JKp_n = [];JKp_p = []; rhs_Kp = [];
    ...
    if ~isempty(eqnSemiNodes)
        ...
        for n1 = eqnSemiNodes.'
            ...
            rhs_Kn(n1) = rhs_Kn(n1) - semiV*chgngrns(n1);
            rhs_Kp(n1) = rhs_Kp(n1) + semiV*chgngrns(n1);
        end
    end
...
%    normUpdate = max([normRes,norm(dV)/norm(Vs(eqnNodes)),abs(currt)/abs(currb)]);
    normUpdate = max([normRes,norm(dV)/norm(Vs(eqnNodes)),norm(dn)/norm(ns(eqnSemiNodes)),norm(dp)/norm(ps(eqnSemiNodes))]);
...
display('End static solution under illum');
