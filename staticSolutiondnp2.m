...
display('Start static solution2 under illum for n & p semicondctor');
...
seminpn = setdiff(semiNodesn,[semiNodesp]);
seminpp = setdiff(semiNodesp,[semiNodesn]);
...
eqnSemiNodes1 = setdiff(semiNodesn,[dirSemiNodes;dirNodes]);
eqnSemiNodes2 = setdiff(semiNodesp,[dirSemiNodes;dirNodes]);
...
Nn_semi1 = length(eqnSemiNodes1);
Nn_semi2 = length(eqnSemiNodes2);
...
while itNr < maxNewtonIt && normUpdate > updateTol
    ...
    JF_n = sparse(JF_n(eqnNodes,eqnSemiNodes1));
    JF_p = sparse(JF_p(eqnNodes,eqnSemiNodes2));
    JF_x = sparse(JF_x(eqnNodes,eqnSemiNodesp));

    JKn_v = sparse(JKn_v(eqnSemiNodes1,eqnNodes));
    JKn_n = sparse(JKn_n(eqnSemiNodes1,eqnSemiNodes1));
    JKn_p = sparse(JKn_p(eqnSemiNodes1,eqnSemiNodes2));
    JKn_x = sparse(JKn_x(eqnSemiNodes1,eqnSemiNodesp));

    JKp_v = sparse(JKp_v(eqnSemiNodes2,eqnNodes));
    JKp_n = sparse(JKp_n(eqnSemiNodes2,eqnSemiNodes1));
    JKp_p = sparse(JKp_p(eqnSemiNodes2,eqnSemiNodes2));
    JKp_x = sparse(JKp_x(eqnSemiNodes2,eqnSemiNodesp));

    JKx_v = sparse(JKx_v(eqnSemiNodesp,eqnNodes));
    JKx_n = sparse(JKx_n(eqnSemiNodesp,eqnSemiNodes1));
    JKx_p = sparse(JKx_p(eqnSemiNodesp,eqnSemiNodes2));
    JKx_x = sparse(JKx_x(eqnSemiNodesp,eqnSemiNodesp));

    rhs_F = sparse(rhs_F(eqnNodes));
    rhs_Kn = sparse(rhs_Kn(eqnSemiNodes1));
    rhs_Kp = sparse(rhs_Kp(eqnSemiNodes2));
...
%    dX = invJacob*rhs;
    ...
    dV = dX(1:Nn);
    dn = dX(Nn+1:Nn+Nn_semi1);
    dp = dX(Nn+Nn_semi1+1:Nn+Nn_semi1+Nn_semi2);
    dxd= dX(Nn+Nn_semi1+Nn_semi2+1:Nn+Nn_semi1+Nn_semi2+Nn_semip);
...
%    normUpdate = max(norm(dV)/norm(Vs(eqnNodes)));
    ...
    ns(eqnSemiNodes1) = ns(eqnSemiNodes1)+dn;
    ps(eqnSemiNodes2) = ps(eqnSemiNodes2)+dp;    
    Xs(eqnSemiNodesp) = Xs(eqnSemiNodesp)+dxd;

    normUpdate = max([normRes,norm(dV)/norm(Vs(eqnNodes)),norm(dn)/norm(ns(eqnSemiNodes1)),norm(dp)/norm(ps(eqnSemiNodes2)),norm(dxd)/norm(Xs(eqnSemiNodesp))]);
...
display('End static solution2 under illum for n & p semicondctor');
